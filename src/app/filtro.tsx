  ...
  const continuar = () => {
    router.push({
      ...
      params: {
        ...
        ate: ate.replace(".", "").replace(",", "."),
        onde: onde.toLowerCase().trim(),
        objetivo: objetivo,
      },
    } as any);
  };
        ...
        <View style={styles.gridTipos}>
          {["Apartamento", "Casa", "Kitnet / Studio", "Quarto", "Cobertura", "Imóvel comercial"].map((t) => (
            <TouchableOpacity key={t} style={[styles.btnTipo, tipo === t && styles.btnTipoAtivo]} onPress={() => setTipo(t)}>
              <Text style={[styles.btnTipoText, tipo === t && styles.btnTipoTextAtivo]} numberOfLines={1}>{t}</Text>
            </TouchableOpacity>
          ))}
        </View>

        <View style={styles.linhaLabel}><View><Text style={styles.label}>Faixa de preço</Text><Text style={styles.labelSmall}>Selecione sua faixa de preço ideal!</Text></View><View style={styles.linhaCinza} /></View>
        <View style={styles.rowPreco}>
          <Text style={styles.deAte}>DE:</Text>
          <View style={styles.inputPreco}><Text style={styles.cifrao}>R$</Text><TextInput value={de} onChangeText={setDe} style={styles.inputPrecoText} keyboardType="numeric" /></View>
          <Text style={styles.deAte}>ATÉ:</Text>
          <View style={styles.inputPreco}><Text style={styles.cifrao}>R$</Text><TextInput value={ate} onChangeText={setAte} style={styles.inputPrecoText} keyboardType="numeric" /></View>
        </View>
        ...
        <View style={styles.overlayMenu}>
          <Animated.View style={[styles.menuLateral, { paddingTop: insets.top + 10, transform: [{ translateX: slideAnim }] }]}>
            <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 30 }}>
              ...
              <TouchableOpacity style={styles.menuItem} onPress={() => irPara("/")}><Ionicons name="home" size={22} color="#000" /><Text style={styles.menuItemText}>Inicio</Text></TouchableOpacity><View style={styles.linha} />
...
const styles = StyleSheet.create({
  ...
  gridTipos: { flexDirection: "row", flexWrap: "wrap", justifyContent: "space-between", marginHorizontal: 16, marginTop: 10, rowGap: 8 },
  btnTipo: { width: "31.5%", borderWidth: 1, borderColor: "#000", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 10, alignItems: "center", justifyContent: "center" },
  btnTipoAtivo: { backgroundColor: "#FF8C00", borderColor: "#FF8C00" },
  btnTipoText: { fontSize: 10, fontWeight: "700", color: "#000", textAlign: "center" },
  ...
  inputPreco: { flex: 1, flexDirection: "row", alignItems: "center", borderWidth: 1.2, borderColor: "#1A5CFF", borderRadius: 10, paddingHorizontal: 10, height: 40, backgroundColor: "#FFF", gap: 4 },
  cifrao: { fontSize: 12, fontWeight: "700", color: "#000" },
  inputPrecoText: { fontSize: 13, fontWeight: "700", color: "#000", flex: 1, paddingVertical: 0 },
  ...
  btnVoltar: { backgroundColor: "#FFF", marginHorizontal: 16, marginTop: 10, height: 44, borderRadius: 12, alignItems: "center", justifyContent: "center", borderWidth: 1.5, borderColor: "#000" },
  btnVoltarText: { color: "#000", fontWeight: "800", fontSize: 14 },
