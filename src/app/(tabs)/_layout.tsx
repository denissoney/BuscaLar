    ...
    <View style={styles.tabBarContainer}>
      <View style={styles.blueBox}>
        ...
        {visibleRoutes.map((route: any) => {
          ...
          if (route.name === "explorar") { iconName = isFocused ? "location" : "location-outline"; label = "EXPLORAR"; }
          if (route.name === "index") { iconName = isFocused ? "home" : "home-outline"; label = "INICIO"; }
          if (route.name === "agendamento") { iconName = isFocused ? "calendar" : "calendar-outline"; label = "AGENDA"; }
          if (route.name === "favoritos") { iconName = isFocused ? "heart" : "heart-outline"; label = "FAVORITOS"; }
          ...
          return (
            <TouchableOpacity key={route.key} onPress={onPress} style={styles.tabItem} activeOpacity={0.9}>
              <View style={[styles.iconBox, isFocused && styles.iconBoxActive]}>
                <Ionicons name={iconName} size={isFocused ? 26 : 22} color={isFocused ? "#fff" : "#B0C4FF"} />
              </View>
              <Text style={[styles.label, { color: isFocused ? "#fff" : "#B0C4FF" }]}>{label}</Text>
            </TouchableOpacity>
          );
        })}
      </View>
...
export default function TabsLayout() {
  return (
    <Tabs tabBar={(props) => <TabBar {...props} />} screenOptions={{ headerShown: false }} initialRouteName="index">
      ...
      {/* ESCONDE O ALUGUÉIS DISPONÍVEIS DA BARRA AZUL */}
      <Tabs.Screen name="imoveis" options={{ href: null, headerShown: false }} />
      <Tabs.Screen name="detalhe" options={{ href: null }} />
    </Tabs>
  );
}
