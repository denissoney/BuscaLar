...
import { View, Text, StyleSheet, Image, TouchableOpacity, ScrollView, PanResponder } from "react-native";
...
const MENSAL = [
  { id: "1", titulo: "Apartamento 2 quartos", preco: "R$ 1.000/mês", local: "Ponta Verde, Maceió", img: "https://images.unsplash.com/photo-1522708323590-d24dbb6b0267?w=400" },
...
export default function Favoritos() {
  ...
  function remover(id: string) {
    if (aba === "Mensal") setMensal((prev) => prev.filter((f) => f.id !== id));
    else setDiarias((prev) => prev.filter((f) => f.id !== id));
  }
  ...
  return (
    <View style={[styles.container, { paddingTop: insets.top }]} {...panResponder.panHandlers}>
      ...
      <View style={styles.content}>
        {/* SETA SEM BOLA + FAVORITOS NO MEIO COM LINHA ORIGINAL LISA */}
        <View style={styles.voltarRow}>
          ...
          <View style={styles.tituloCentro}>
            ...
            <Svg height={4} width={75} style={{ marginTop: 3 }}>
              <Polygon points="0,0 75,0 75,1 0,4" fill="#FF8C00" />
            </Svg>
          </View>
        ...
        <View style={styles.abasContainer}>
          <TouchableOpacity style={[styles.aba, aba === "Mensal" && styles.abaAtiva]} onPress={() => setAba("Mensal")} activeOpacity={0.8}>
            <Text style={[styles.abaTexto, aba === "Mensal" && styles.abaTextoAtivo]}>Mensal</Text>
          </TouchableOpacity>
          <TouchableOpacity style={[styles.aba, aba === "Diarias" && styles.abaAtiva]} onPress={() => setAba("Diarias")} activeOpacity={0.8}>
            <Text style={[styles.abaTexto, aba === "Diarias" && styles.abaTextoAtivo]}>Diárias</Text>
          </TouchableOpacity>
        </View>

        <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 100, paddingTop: 6 }}>
          {lista.map((item) => (
            <View key={item.id} style={[styles.card, styles.sombra]}>
              ...
              <TouchableOpacity style={styles.btnHeart} onPress={() => remover(item.id)} activeOpacity={0.7}>
                <Ionicons name="heart" size={20} color="#FF8C00" />
              </TouchableOpacity>
            </View>
          ))}
          ...
          {lista.length === 0 && (
            <View style={styles.vazio}>
              <Ionicons name="heart-outline" size={55} color="#D0D0D0" />
              <Text style={styles.vazioTitulo}>Nenhum favorito</Text>
              <Text style={styles.vazioTexto}>Você ainda não adicionou nenhum imóvel aos favoritos.</Text>
            </View>
          )}
        </ScrollView>
      </View>
