...
import {
  ...
  ScrollView, Image, FlatList, useWindowDimensions, Alert, PanResponder,
} from "react-native";
...
export default function Home() {
  ...
  // RECEBE FILTRO DA TELA DE FILTRO
  ...
  // SE VIER DO FILTRO, APLICA AUTOMATICO
  useEffect(() => {
    if (params.tipo || params.de || params.ate || params.onde) {
      ...
      if (params.onde) {
        ...
        filtrados = filtrados.filter(i =>
          i.local.toLowerCase().includes(local) ||
          i.nome.toLowerCase().includes(local)
        );
      }
      if (params.de) {
      ...
      if (filtrados.length === 0) Alert.alert("BuscaLar", "Nenhum imóvel encontrado com esse filtro.");
    }
  ...
  function buscar() {
    ...
    if (localBusca!== "") {
      filtrados = filtrados.filter(item => item.local.toLowerCase().includes(localBusca) || item.nome.toLowerCase().includes(localBusca));
    }
    if (valor.trim()!== "") {
          ...
          {imoveis.map((item) => (
            <TouchableOpacity key={item.id} style={[styles.card, { width: isTablet? (width - filtroPadding * 2 - 12) / 2 : "100%" }]} onPress={() => router.push("/perfil-proprietario" as any)} activeOpacity={0.8}>
              ...
              <View style={styles.cardInfo}>
                ...
                <Text style={styles.cardPreco}>R$ {item.valor} - {item.quartos} quarto(s) • {item.local} - {item.tipo}</Text>
              </View>
            </TouchableOpacity>
          ))}
        </View>
